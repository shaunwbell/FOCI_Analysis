...
data = pd.read_csv(args.sourcefile)
data['ID'].fillna('')

for index,row in data.iterrows():
  ICOADSdb.add_to_DB(table='ICOADS_SST',YR=row.YR,MO=row.MO,DY=row.DY,HR=row.HR,LAT=row.LAT,LON=row.LON,
  						ID=row.ID,SST=row.SST)
  if index % 1000 == 0:
