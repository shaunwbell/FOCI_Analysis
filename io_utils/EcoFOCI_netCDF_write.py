...
class NetCDF_Copy_Struct(object):
    ...
    def add_data(self, data=None, trim_index=None, depth_index=0):
        """ """
        
        for ind, varname in enumerate(data.keys()):
            if not varname in ['time','time2','lat','lon','depth','latitude','longitude']:
                di = self.rec_vars.index(varname)
                self.var_class[di][:] = data[varname][trim_index,depth_index,0,0]
...
class NetCDF_Create_Timeseries_CF(object):
    """ Class instance to generate a NetCDF file.  
    ...
    Use this to create a nc file with all default values
        ncinstance = NetCDF_Create_Timeseries_CF()
        ncinstance.file_create()
        ncinstance.sbeglobal_atts()
        ncinstance.dimension_init()
        ncinstance.variable_init()
    ...
    def file_create(self):
            rootgrpID = Dataset(self.savefile, NetCDF_Create_Timeseries_CF.nc_read, 
                                format=NetCDF_Create_Timeseries_CF.nc_format)
            self.rootgrpID = rootgrpID
            return ( rootgrpID )
    ...
    def variable_init(self, EPIC_VARS_dict):
        ...
        var_class = []
        var_class.append(self.rootgrpID.createVariable(rec_vars[0], rec_var_type[0], self.dim_vars[0]))#time
        ...
        for i, v in enumerate(rec_vars[4:]):  #1D coordinate variables
            var_class.append(self.rootgrpID.createVariable(rec_vars[i+4], rec_var_type[i+5], self.dim_vars))
