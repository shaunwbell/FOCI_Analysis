...
__keywords__ = 'NARR','station_1','3hr filtered', 'U,V','Winds', 'Gulf of Alaska'
...
sta_long = [153., 155.]
"""
station_name = [ 'SS_Entrance_SW','SS_Entrance_NE']
sta_lat = [57.6, 57.6,]
sta_long = [155.2, 155.]
...
station_2 = sphered.nearest_point([sta_lat[1],-1 * sta_long[1]],lat_lon['lat'],lat_lon['lon'], '2d')
station_1_modelpt = [lat_lon['lat'][station_1[3],station_1[4]],lat_lon['lon'][station_1[3],station_1[4]]]
station_2_modelpt = [lat_lon['lat'][station_2[3],station_2[4]],lat_lon['lon'][station_2[3],station_2[4]]]

print "station_1 nearest point to %s, %s which is lat:%s , lon:%s" \
    % (sta_lat[0], sta_long[0], station_1_modelpt[0], station_1_modelpt[1])
    
print "station_2 nearest point to %s, %s which is lat:%s , lon:%s" \
    % (sta_lat[1], sta_long[1], station_2_modelpt[0], station_2_modelpt[1])    
...
for yy in years:
    ...
    station_2_data = from_netcdf_1dsplice(infile, None, station_2[3], station_2[4])

    #filter data
    station_1u_f = triangle_smoothing(station_1_data['uwnd'])
    station_2u_f = triangle_smoothing(station_2_data['uwnd'])
    ...
    station_2v_f = triangle_smoothing(station_2_data['vwnd'])
    ...
    if save_to_nc:
        ...
        write2epic( outfile, station_name[1], [epic_time, epic_time1], station_1_modelpt, [station_1u_f, station_1v_f])
        outfile = 'data/NARR_' + station_name[1] + '_' + str(yy) + '.nc'
        print "Writing to Epic NetCDF " + outfile
        write2epic( outfile, station_name[0], [epic_time, epic_time1], station_2_modelpt, [station_2u_f, station_2v_f])
...
if plot_geoloc:
    ...
    m = Basemap(resolution='i',projection='merc', llcrnrlat=55, \
        urcrnrlat=58,llcrnrlon=-158,urcrnrlon=-150, lat_ts=45)
    


    # Mooring Data
    x_moor, y_moor = m([-1. * sta_long[0], -1. * sta_long[1]],sta_lat)
    x_close, y_close = m([station_1_modelpt[1],station_2_modelpt[1]], [station_1_modelpt[0],station_2_modelpt[0]])
    ...
    plt.savefig('images/Shelikof_region.png', bbox_inches='tight', dpi = (100))
    plt.close()
