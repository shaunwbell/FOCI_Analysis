...
"""
 ...
 Note:
 -----
 SS data is in text format and lacking geolocation information
 
 See Bond and Stabeno 1998 - 
 Analysis of Surface Winds in Shelikof Strait, Alaksa, Using Moored Buoy Observations
...
__keywords__ = 'NARR', 'NCEP V2','Shelikof Strait', 'power law', 'user defined time comparison', 'Winds', 'Bering Sea'
...
def date2pydate(file_time, file_time2=None, file_flag='EPIC'):
    ...
    if file_flag == 'EPIC':
        ...
        """yymmdd"""
        python_time = [datetime.datetime.strptime(str(x),'%y%m%d').toordinal() for x in file_time]
        python_time = python_time + file_time2 / 2400.
    else:
...
MooringFile = '/Users/bell/Data_Local/from_nick/94ssmoorings/'+moor_local_name+'.dat'
MooringData=csvread(MooringFile)

MooringTime = date2pydate(MooringData['DAT'], MooringData['TIM'], file_flag='dat')
...
MooringDaily_uwnd = hourly_2_ave(NARRTime.min(),NARRTime.max(), MooringTime, \
    wind_power_law(MooringData['WU'], correction=True), time_base=time_bin)
MooringDaily_vwnd = hourly_2_ave(NARRTime.min(),NARRTime.max(), MooringTime, \
    wind_power_law(MooringData['WV'], correction=True), time_base=time_bin)
...
(NCEPalong, NCEPcross) = rotate_coord(135., NCEP_wind_mag, NCEP_wind_dir_math)
(MOORalong, MOORcross) = rotate_coord(135., Mooring_wind_mag, Mooring_wind_dir_math)
...
if plot_loc:
    ...
    fig = plt.figure()
    ax = plt.subplot(111)
    m = Basemap(resolution='i',projection='merc', llcrnrlat=56, \
        urcrnrlat=62,llcrnrlon=-158,urcrnrlon=-140, lat_ts=45)
