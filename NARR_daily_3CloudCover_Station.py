...
def write2epic( file_name, stationid, time, lat_lon, data ):
        ...
        ncinstance.add_coord_data(time1=time[0], time2=time[1], latitude=lat_lon[0], longitude=-1 * lat_lon[1], \
            depth_level=10. )
        ncinstance.add_data('LC_001', data[0])
        ncinstance.add_data('MC_002', data[1])
        ncinstance.add_data('HC_003', data[2])
        ncinstance.close()
...
infile = [NARR + 'lcdc.2016.nc'] #used just to get grid sections
...
for yy in years:
    ...
    infile = NARR + 'lcdc.'+ str(yy) + '.nc'
    print "Working on file " + infile
    station_1_data = from_netcdf_1dsplice(infile, None, station_1[3], station_1[4])
    station_1u_f = station_1_data['lcdc']
    
    # retrieve only these location's data
    infile = NARR + 'mcdc.'+ str(yy) + '.nc'
    print "Working on file " + infile
    station_1_data = from_netcdf_1dsplice(infile, None, station_1[3], station_1[4])
    station_1v_f = station_1_data['mcdc']

    # retrieve only these location's data
    infile = NARR + 'hcdc.'+ str(yy) + '.nc'
    print "Working on file " + infile
    station_1_data = from_netcdf_1dsplice(infile, None, station_1[3], station_1[4])
    station_1at = station_1_data['hcdc'] #Kelvin
