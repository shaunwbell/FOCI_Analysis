...
def nearest_point(origin, latpoints, lonpoints, grid='1d', threshold=10):
    ...
    if grid is '1d':
        ...
        for i, latrow in enumerate(latpoints):
            for j, lonrow in enumerate(latrow):
                if np.isnan(latpoints[i,j]) or np.isnan(lonpoints[i,j]):
                    dist[i,j] = np.nan
                else:
                    dist[i,j] = distance(origin,[latpoints[i,j],lonpoints[i,j]])

                if dist[i,j] <= threshold:
                    lati, loni = np.where(dist == np.nanmin(dist) )
                    
                    return (np.nanmin(dist), latpoints[lati[0]][loni[0]], lonpoints[lati[0]][loni[0]], lati[0], loni[0] )