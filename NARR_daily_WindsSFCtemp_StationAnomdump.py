...
def from_netcdf_1dsplice(infile, height_ind, lat_ind, lon_ind):
    ...
    ncdata = ncutil.ncreadfile_dic_slice(
        nchandle, params, height_ind=height_ind, lat_ind=lat_ind, lon_ind=lon_ind
    )
    ncutil.ncclose(nchandle)
...
def csvread(ifile):
    ...
    return {
        "DAT": np.array(date, int),
        "TIM": np.array(time, float),
        "WU": np.array(uwnd, float),
...
def write2epic(file_name, stationid, time, lat_lon, data):
    ...
    ncinstance.PMELglobal_atts(
        Station_Name=stationid, file_name=(__file__.split("/")[-1])
    )
...
def write2epic_cf(file_name, stationid, time, lat_lon, data):
    ...
    ncinstance.variable_init()
    ncinstance.add_coord_data(
        time=time, latitude=lat_lon[0], longitude=-1 * lat_lon[1], depth_level=10.0
    )
    ncinstance.add_data("WU_422", data[0])
    ncinstance.add_data("WV_423", data[1])
    ncinstance.add_data("AT_21", data[2])
...
def date2pydate(file_time, file_time2=None, file_flag="EPIC"):
    ...
    if file_flag == "EPIC":
        ...
        try:  # if input is an array
            ...
            pyfrac = file_time2 / (1000.0 * 60.0 * 60.0 * 24.0)  # milliseconds in a day
        ...
        python_time = file_time / 24.0 + base_date
    elif file_flag == "NCEP":
        """ Hours since 1800-1-1"""
        base_date = datetime.datetime.strptime("1800-01-01", "%Y-%m-%d").toordinal()
        python_time = file_time / 24.0 + base_date
...
def pydate2EPIC(file_time):
    ...
    time1 = (
        np.floor(file_time) + offset
    )  # truncate to get day and add 2440000 for true julian day

    time2 = (file_time - np.floor(file_time)) * (
        1000.0 * 60.0 * 60.0 * 24.0
    )  # milliseconds since 0000GMT
...
def etopo5_data():
    ...
    topoin, lons = shiftgrid(0.0, topoin, lons, start=False)  # -360 -> 0
...
parser.add_argument(
    "--DataPath", metavar="DataPath", type=str, help="full path to alternate file"
)
...
if args.DataPath:
    ...
    NARR = "/Users/bell/in_and_outbox/data_sets/reanalyis_data/NARR/"

infile = [NARR + "uwnd.10m.2017.nc"]  # used just to get grid sections
...
station_1 = sphered.nearest_point(
    [sta_lat[0], -1 * sta_long[0]], lat_lon["lat"], lat_lon["lon"], "2d"
)
station_1_modelpt = [
    lat_lon["lat"][station_1[3], station_1[4]],
    lat_lon["lon"][station_1[3], station_1[4]],
]
...
for yy in years:
    ...
    # uwnd
    infile = NARR + "uwnd.10m.day.ltm.nc"
    ...
    # retrieve only these location's data
    # vwnd
    infile = NARR + "vwnd.10m.day.ltm.nc"
    print "Working on file " + infile
    station_1_data = from_netcdf_1dsplice(infile, None, station_1[3], station_1[4])
    ...
    if save_to_nc:
        # write to NetCDF
        outfile = "data/NARR_" + station_name[0] + "10m.day.ltm.nc"
        print "Writing to Epic NetCDF " + outfile

        write2epic(
            outfile,
            station_name[0],
            [epic_time, epic_time1],
            station_1_modelpt,
            [station_1u_f, station_1v_f, station_1at],
        )
